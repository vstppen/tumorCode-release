...
{   
    ...
    generate_boundary_points(filename_ctrlPts_, filename_ctrl_nxny_, filename_bdryPts_, recordOrNot_);
...
CartesianGridAndControlPoints::~CartesianGridAndControlPoints()
{
    ...
    freeIntMatrix(int_node_ij_to_l, I + 1);
...
void CartesianGridAndControlPoints::generate_boundary_points(const std::string& filename_ctrlPts, const std::string& filename_ctrl_nxny, const std::string& filename_bdryPts, bool recordOrNot_) 
{
    bdry_delta = M_2PI / M;
...
{   
    ...
    int_node_ij_to_l = createIntMatrix(I + 1, J + 1);
    for (int i = 1; i < I; i++) {
        for (int j = 1; j < J; j++) {
            if (interior[i][j]) {
                int_node_index[count][0] = i; 
                int_node_index[count][1] = j; 
                int_node_ij_to_l[i][j] = count;
                count++;
            } else {
                int_node_ij_to_l[i][j] = -1;
            }
        }
    }
...
void CartesianGridAndControlPoints::computeJumps2(double x, double y, double s, double tx, double ty, double nx, double ny, double d_psi, double dd_phi, const double du[2], double ddu[3], double lambda, double phi)
{
    double ddX[2], dn[2]; 
    ...
    mat[0][0] = txtx;   // eq (55b)
    mat[0][1] = tyty;   // eq (55b)
    mat[0][2] = txty + txty;    // eq (55b)

    mat[1][0] = nxtx;   // eq (55a)
    mat[1][1] = nyty;   // eq (55a)
    mat[1][2] = nxty + nytx;    // eq (55a)
    ...
    rhs[0] = dd_phi - ddx * du[0] - ddy * du[1];    // eq (55b)
    rhs[1] = d_psi - dnx * du[0] - dny * du[1];     // eq (55a)
    rhs[2] = lambda * phi;   // !!!2

    bool status = solveByQRdecomposition<3>(mat, rhs, ddu, 3);      // ddu[3]: u_xx, u_yy, u_xy
}
...
void CartesianGridAndControlPoints::computeJumps(const double *phi_vec, const double *psi_vec, int K,
                  ...
                  double nx, double ny, double &j0, double j1[2], double j2[3], double lambda) 
{
    double psi, d_psi; 
    ...
    computeJumps2(x0, y0, s, tx, ty, nx, ny, d_psi, dd_phi, jmp1, jmp2, lambda, phi); 
...
void CartesianGridAndControlPoints::computeJumps2(double x, double y, double s, double tx, double ty, double nx, double ny, double f, double ddu[3], double lambda, bool interiorOrNot)
{   
    double ddX[2], dn[2]; 
...
void CartesianGridAndControlPoints::computeJumps(double x0, double y0, double s, double tx, double ty, double nx, double ny, double f, double jmp2[3], double lambda, bool interiorOrNot)
{
    computeJumps2(x0, y0, s, tx, ty, nx, ny, f, jmp2, lambda, interiorOrNot);
}


void CartesianGridAndControlPoints::makeCorrection(const double *phi, const double *psi, int K, double **b, double lambda)
{
    double d1, d2, c;
    ...
    for (int ell = 0; ell < M0; ell++) {
        ...
        computeJumps(phi, psi, K, p, Grid_y[j], s, tx, ty, nx, ny, j0, j1, j2, lambda); 

        d1 = Grid_x[i + 1] - p;
        d2 = 0.5 * d1 * d1;
        c = j0 + j1[0] * d1 + j2[0] * d2; 
        if (interior[i + 1][j]) {
            b[i][j] -= -c;  
            // !!!8 
            // The parameter kappa does not affect this part because it only modifies the diagonal elements of the difference matrix,  
            // whereas the correction term in Eq. (65) involves only the summation over j ≠ i with j ∈ 𝓘(i).  
            //  
            // The sign change occurs because the original kfbi code solves -Δu = f,  
            // while this implementation solves Δu - λu = f.  
            // In other words, the sign flip in the Laplacian causes the off-diagonal elements of the difference matrix  
            // to become the negation of their previous values. Specifically, in Eq. (65), a_{i,j} changes sign accordingly.  
        } else {
            b[i][j] += -c;
        }
    ...
    for (int ell = M0; ell < M0 + M1; ell++) {
        ...
        computeJumps(phi, psi, K, Grid_x[i], q, s, tx, ty, nx, ny, j0, j1, j2, lambda); 

        d1 = Grid_y[j + 1] - q; 
        d2 = 0.5 * d1 * d1; 
        c = j0 + j1[1] * d1 + j2[1] * d2; 
        if (interior[i][j + 1]) {
            b[i][j] -= -c; 
        } else {
            b[i][j] += -c;
...
void CartesianGridAndControlPoints::makeCorrection(double (*F)(double, double), double **b, double lambda, bool interiorOrNot)
{
  ...
  for (int ell = 0; ell < M0; ell++) {
    ...
    // }
    computeJumps(p, Grid_y[j], s, tx, ty, nx, ny, f, jmp2, lambda, interiorOrNot);
    // if (ell == 1) {
    //     std::cout << jmp2[0] << " " << jmp2[1] << " " << jmp2[2] << " " << std::endl;
    // }
  ...
  for (int ell = M0; ell < M0 + M1; ell++) {
    ...
    //     std::cout << jmp2[0] << " " << jmp2[1] << " " << jmp2[2] << " " << std::endl;
    // }
    computeJumps(Grid_x[i], q, s, tx, ty, nx, ny, f, jmp2, lambda, interiorOrNot);
    // if (ell == M0 + 1) {
    //     std::cout << jmp2[0] << " " << jmp2[1] << " " << jmp2[2] << " " << std::endl;
    // }
...
void CartesianGridAndControlPoints::extractDirichletBoundaryData(double **u, const double *phi_vec, const double *psi_vec, double *bdry_u_vec, int K, double lambda) 
{
    double jump_u[6], bdry_u[12];
    ...
    for (int k = 0; k < K; k++) {
        ...
        computeJumps(phi_vec, psi_vec, K, xi, eta, bdry_theta[k], tx, ty, nx, ny, j0, j1, j2, lambda); 

        jump_u[0] = j0;     jump_u[1] = j1[0];   jump_u[2] = j1[1];
        jump_u[3] = j2[0];  jump_u[4] = j2[1];   jump_u[5] = j2[2];

        computeBoundaryValues(u, xi, eta, jump_u, bdry_u);

        bdry_u_vec[k] = bdry_u[0]; 
    }
}

void CartesianGridAndControlPoints::extractDirichletBoundaryData(double **w, double (*F)(double, double), double *bdry_w_vec, int K, double lambda, bool interiorOrNot) 
{
  ...
  for (int k = 0; k < K; k++) {
    ...
    computeJumps(xi, eta, bdry_theta[k], tx, ty, nx, ny, f, j2, lambda, interiorOrNot); 
...
void CartesianGridAndControlPoints::extractNeumannBoundaryData(double **u, const double *phi_vec, const double *psi_vec, double *bdry_un_vec, int K, double lambda)
{
    double jump_u[6], bdry_u[12];
    ...
    for (int k = 0; k < K; k++) {
        ...
        bdry_un_vec[k] = bdry_u[1] * nx + bdry_u[2] * ny; 
    }
}

void CartesianGridAndControlPoints::extractNeumannBoundaryData(double **w, double (*F)(double, double), double *bdry_wn_vec, int K, double lambda, bool interiorOrNot)
{
    double j2[3], jump_w[6], bdry_w[12];
