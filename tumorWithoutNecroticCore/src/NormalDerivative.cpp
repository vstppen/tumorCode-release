...
void computeBoundaryValues(double x, double y, double** coord, double* grid_u, int count, double& u, double& ux, double& uy) {
    ...
    MatrixMulMatrix(AT, A, ATA, 6, count, 6);
    MatrixMulVector(AT, b, ATb, 6, count);
...
void getNormalDerivatives(CartesianGridAndControlPoints* G, double (*g)(double, double), double** u, double* v)
{
    ...
    // dxOnCartesianGrid(G, g, u, ux); 
    // dyOnCartesianGrid(G, g, u, uy); 

    // extern double U_Poisson_x(double x0, double y0);
    // extern double U_Poisson_y(double x0, double y0);
    // for (int i = 0; i < G->I + 1; i++) {
    //     for (int j = 0; j < G->J + 1; j++) {
    //         if (G->interior[i][j]) {
    //             ux[i][j] = U_Poisson_x(G->Grid_x[i], G->Grid_y[j]);
    //             uy[i][j] = U_Poisson_y(G->Grid_x[i], G->Grid_y[j]);
    //         }
    //     }
    // }

    // printMatrix(uy, G->I + 1, G->J + 1);

    extractboundaryData(G, g, u, ux_bdry, uy_bdry);

    // printVector(ux_bdry, G->M);
    // std::cout << ux_bdry[103] << std::endl;
