...
int extractZeroLevelSet(double** u, int Nx, int Ny, double* x, double* y, Point* pts) {
    ...
    for (int i = 0; i < Nx; ++i) {
        for (int j = 0; j < Ny; ++j) {
            if (std::abs(u[i][j]) < 1.e-12) {
                bool hasPositiveNeighbor = false;

                if (i > 0     && u[i - 1][j] > 1.e-12) hasPositiveNeighbor = true;
                if (i < Nx-1  && u[i + 1][j] > 1.e-12) hasPositiveNeighbor = true;
                if (j > 0     && u[i][j - 1] > 1.e-12) hasPositiveNeighbor = true;
                if (j < Ny-1  && u[i][j + 1] > 1.e-12) hasPositiveNeighbor = true;
