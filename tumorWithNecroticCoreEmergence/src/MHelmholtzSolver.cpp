...
double BoundaryIntegralMHelmholtz(CartesianGridAndControlPoints0* G, double* phi, int idx_t) 
{
    double result = 0.0;
    for (int i = 0; i < G->M; i++) {
...
double BoundaryIntegralMHelmholtz(CartesianGridAndControlPoints0* G, double* phi, int idx_t, double** K) 
{
    double result = 0.0;
    for (int i = 0; i < G->M; i++) {
...
void solveBIEMHelmholtz(CartesianGridAndControlPoints0* G, double* phi, double** w)
{
...
double computeNearBoundaryValuesMHelmholtz(double** coor, double* u, double x, double y, int count) {
    ...
    MatrixMulMatrix_L(AT, A, ATA, 6, valid_count, 6);
    MatrixMulVector_L(AT, b, ATb, 6, valid_count);
